...
finfo = dict(
    ...
    numpy = dict(
        ...
        func_str = func_name,
        arch = "CPU",
        arg_str = "nit, np_u, np_v, dt, dx, dy, np_p, rho, nu, F",
    ...
    numba = dict(
        ...
        arch = "CPU",
        arg_str = "nit, nb_u, nb_v, dt, dx, dy, nb_p, rho, nu, F",
    ...
    pythran = dict(
        ...
        func_str = func_name,
        arch = "CPU",
        arg_str = "nit, pt_u, pt_v, dt, dx, dy, pt_p, rho, nu, F",
    ...
    cupy = dict(
        ...
        func_str = func_name,
        arch = "GPU",
        arg_str = "nit, gu, gv, dt, dx, dy, gp, rho, nu, F",
    ...
    dace_cpu = dict(
        ...
        func_str = func_name,
        arch = "CPU",
        arg_str = "nx=nx, ny=ny, nit=nit, u=dc_u, v=dc_v, dt=dt, dx=dx, dy=dy, "
                  "p=dc_p, rho=rho, nu=nu, F=F",
    ...
    dace_gpu = dict(
        ...
        func_str = func_name,
        arch = "GPU",
        arg_str = "nx=nx, ny=ny, nit=nit, u=gu, v=gv, dt=dt, dx=dx, dy=dy, "
                  "p=gp, rho=rho, nu=nu, F=F",
        setup_str = "gu, gv, gp = cp.asarray(u), cp.asarray(v), cp.asarray(p)",
        report_str = "DaCe GPU",
        out_args = ("gu", "gv", "gp")
    )
...
# # Argument strings
# numpy_arg_str = "nit, np_u, np_v, dt, dx, dy, np_p, rho, nu, F"
# numba_arg_str = "nit, nb_u, nb_v, dt, dx, dy, nb_p, rho, nu, F"
...
if __name__ == "__main__":
    ...
    #         try:
    #             from channel_flow_numpy import channel_flow as np_impl
