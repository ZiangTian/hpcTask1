...
@dc.program
def conv2d(input: dc.float32[S0, S1, S2, S3],
           weights: dc.float32[S4, S4, S3, S5]):
...
def maxpool2d(x: dc.float32[S0, S1, S2, S3]):
    ...
    #     dtype=x.dtype)
    output = np.ndarray(
        [S0, S1 // 2, S2 // 2, S3], dtype=np.float32)
...
def lenet5(input: dc.float32[N, H, W, 1], conv1: dc.float32[5, 5, 1, 6],
           ...
           fc2b: dc.float32[84], fc3w: dc.float32[84, 10], fc3b: dc.float32[10]):
...
# LeNet-5 Convolutional Neural Network (inference mode)
@dc.program
def lenet5_gpu(out:dc.float32[N, 10], input: dc.float32[N, H, W, 1],
               conv1: dc.float32[5, 5, 1, 6], conv1bias: dc.float32[6],
               conv2: dc.float32[5, 5, 6, 16], conv2bias: dc.float32[16],
               fc1w: dc.float32[C_before_fc1, 120], fc1b: dc.float32[120],
               fc2w: dc.float32[120, 84], fc2b: dc.float32[84],
