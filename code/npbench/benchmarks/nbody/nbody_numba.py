...
def getAcc_obj( pos, mass, G, softening ):
    """
...
def getAcc_obj_par( pos, mass, G, softening ):
    """
    Calculate the acceleration on each particle due to Newton's Law 
    pos  is an N x 3 matrix of positions
    mass is an N x 1 vector of masses
...
@nb.jit(nopython=False, forceobj=True, parallel=False, fastmath=True)
def getEnergy_obj( pos, vel, mass, G ):
    """
...
@nb.jit(nopython=False, forceobj=True, parallel=True, fastmath=True)
def getEnergy_obj_par( pos, vel, mass, G ):
    """
    Get kinetic energy (KE) and potential energy (PE) of simulation
    pos is N x 3 matrix of positions
    vel is N x 3 matrix of velocities
...
def object_mode(mass, pos, vel, N, Nt, dt, G, softening):
    ...
    for i in range(Nt):
        ...
        KE[i+1], PE[i+1]  = getEnergy_obj( pos, vel, mass, G )
    
    return KE, PE


@nb.jit(nopython=False, forceobj=True, parallel=True, fastmath=True)
def object_mode_parallel(mass, pos, vel, N, Nt, dt, G, softening):
    ...
    # calculate initial gravitational accelerations
    acc = getAcc_obj_par( pos, mass, G, softening )

    # calculate initial energy of system
    KE = np.ndarray(Nt+1, dtype=np.float64)
    PE = np.ndarray(Nt+1, dtype=np.float64)
    KE[0], PE[0]  = getEnergy_obj_par( pos, vel, mass, G )
    ...
    for i in range(Nt):
        ...
        # get energy of system
        KE[i+1], PE[i+1]  = getEnergy_obj_par( pos, vel, mass, G )
...
@nb.jit(nopython=True, parallel=False, fastmath=True)
def nopython_mode(mass, pos, vel, N, Nt, dt, G, softening):
